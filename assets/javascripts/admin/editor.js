/**
 * 2015-07-26
 * Стандартный Markdown.Editor содержит дефект функции firstByClass,
 * который нормальным способом невозможно устранить,
 * потому что эта функция имеет локальную область видимости:
 * https://github.com/discourse/discourse/blob/v1.4.0.beta6/app/assets/javascripts/discourse/lib/Markdown.Editor.js#L311
 ...
 * Как можно заметить, эта функция игнорирует свой параметр containerElement,
 * и, таким образом, хотя номинально заявлена поддержка
 * одновременно нескольких редакторов на общей странице,
 * на самом деле она не работает: все панели кнопок, в частности,
 * достаются первому редактору, а остальним — шиш.
 * По этой причине мне приходится (временно?) использовать свой класс.
 */
(function () {
    ...
    Markdown.DfEditor = function (markdownConverter, idPostfix, options) {
    ...
    function firstByClass(doc, containerElement, className) {
		return $('.' + className, $(containerElement)).get(0);
    }
